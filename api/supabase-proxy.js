...
async function saveUserNotificationsToSupabase(userId, notifications) {
  ...
  try {
    ...
    console.warn('[supabase-proxy] saveUserNotificationsToSupabase error:', err.message);
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'get_notifications' || action === 'get_user_notifications') {
      const targetUserId = body.userId || body.user_id || null;
      const notifs = await getUserNotificationsFromSupabase(targetUserId);
      return res.status(200).json({ success: true, data: notifs });
    }
    ...
    if (action === 'add_notification' || action === 'add_user_notification') {
      const targetUserId = body.userId || body.user_id || body.notification?.userId;
      const item = body.notification || body.data || body;
    ...
    if (action === 'mark_notifications_read') {
      const targetUserId = body.userId || body.user_id;
      const notifId = body.notificationId || body.id || null;
    ...
    if (action === 'check_nickname_cooldown') {
      ...
      // Cek public.users by email jika id tidak menemukan
      if (targetEmail) {
        try {
    ...
    if (action === 'update_nickname') {
      ...
      if ((!targetUserId && !targetEmail) || !newNickname) {
        return res.status(400).json({ success: false, error: 'Identitas user dan nickname baru diperlukan.' });
      }
      ...
      // 1. Cek riwayat pergantian sebelumnya dari auth user metadata dan tabel users (baik by id maupun by email)
      let lastChangeTime = null;
      ...
      // 2. Update kolom name & nickname_updated_at di public.users (coba id dulu, jika tidak ada gunakan email)
      ...
      if (updateAttempt1?.data) {
        ...
        // Fallback jika kolom nickname_updated_at belum ada di schema table users
      ...
      // 3. Update metadata di auth.users (PASTI tersimpan secara cloud untuk semua device)
      if (matchedAuthId) {
        try {
